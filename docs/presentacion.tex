...
\begin{frame}{Comandos Docker}
  ...
  \begin{tabular}{|l|l|}
    ...
    Iniciar (nuevo) & \texttt{docker run -d -p 5000:5000 --name mango-api mango-api} \\
    Iniciar (existente) & \texttt{docker start mango-api} \\
    Ver corriendo & \texttt{docker ps} \\
...
\begin{frame}{Demo — Resultado real}
  ...
  \begin{columns}
    ...
    \begin{beamercolorbox}[wd=\textwidth,center,rounded=true]{block body}
      \textbf{ResNet50}\\
      \texttt{"tipo": "Tipo\_4"}\\
    ...
    \begin{beamercolorbox}[wd=\textwidth,center,rounded=true]{block body}
      ...
      \texttt{"tipo": "Tipo\_4"}\\
      \texttt{"exportable": false}\\
      \texttt{"probabilidad": 89.12}
    \end{beamercolorbox}
  \end{columns}

  \vspace{1em}
  \footnotesize Ambos modelos coinciden en la clasificación. ResNet50 tiene mayor confianza por el Transfer Learning.
\end{frame}
